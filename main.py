...
def setup_nft_rules(vpn_iface):
    ...
    subprocess.run(["nft", "add", "table", "ip", "nat"], capture_output=True, timeout=5)
    subprocess.run(
    ...
    subprocess.run(
        ["nft", "add", "rule", "ip", "nat", "postrouting", "oifname", vpn_iface, "masquerade"],
        capture_output=True, timeout=5
    )
    subprocess.run(["nft", "add", "table", "ip", "filter"], capture_output=True, timeout=5)
    subprocess.run(
    ...
    subprocess.run(
        ["nft", "add", "rule", "ip", "filter", "forward", "iifname", vpn_iface, "accept"],
        capture_output=True, timeout=5
    )
    subprocess.run(
        ["nft", "add", "rule", "ip", "filter", "forward", "oifname", vpn_iface, "accept"],
        capture_output=True, timeout=5
    )
...
def main():
    ...
    # НЕ удаляем старые маршруты — только синхронизируем известные
    # cleanup_old_routes(VPN_INTERFACE) - убрано
    ...
    # Синхронизируем сохранённые IP при старте (только добавляем, не удаляем)
    managed_ips = set()
    ...
    if managed_ips:
        ...
        rm.sync_routes(managed_ips, skip_deletes=True)

    # Запускаем DNS
    dns_thread = threading.Thread(target=dns.start, daemon=True)
    dns_thread.start()

    # Даём DNS-серверу время запуститься, потом сразу резолвим домены
    time.sleep(2)
    print("[MAIN] Первоначальный ре-резолвинг доменов...")
    try:
        dns._refresh_all_managed()
    except Exception as e:
        print(f"[MAIN] Ошибка при первом ре-резолвинге: {e}")
